...
function categoryLabel(category: string) {
  return category === "success-stories" ? "成果案例" : category;
}
...
function Visual({ post, variant }: { post: Post; variant: number }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-[#032a72]" aria-hidden="true">
      <div className="absolute inset-0 opacity-70 [background-image:linear-gradient(rgba(72,112,255,0.2)_1px,transparent_1px),linear-gradient(90deg,rgba(72,112,255,0.2)_1px,transparent_1px)] [background-size:38px_38px]" />
      <div className={`absolute rounded-full border-[32px] border-[#4870ff]/30 shadow-[0_0_90px_rgba(72,112,255,0.25)] ${variant % 2 === 0 ? "-right-10 -top-12 h-56 w-56" : "-bottom-16 -left-12 h-64 w-64"}`} />
      <div className="absolute right-7 top-7 font-mono text-[10px] tracking-[0.28em] text-[#b8c6ff]">0{variant + 1} / MEDBOT</div>
    </div>
  );
}
...
function TopicCard({ post, large = false, variant = 0 }: { post: Post; large?: boolean; variant?: number }) {
  return (
    <Link href={href(post)} className={`group relative block overflow-hidden bg-[#032a72] ${large ? "min-h-[430px] lg:min-h-[540px]" : "min-h-[255px]"}`}>
      ...
      <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
        <span className="inline-flex bg-[#4870ff] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.1em] text-white">{categoryLabel(post.category) || "MEDBOT"}</span>
...
export default function TopicLanding({ title, description, kind, posts }: { title: string; description: string; kind: TopicKind; posts: Post[] }) {
  ...
  return (
    <div className="min-h-full bg-white text-black">
      <section className="border-b border-[#d8e0ee] bg-[#f3f6fb]">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16">
          <div className="mb-4 flex items-center gap-2 font-mono text-[11px] tracking-[0.2em] text-[#032a72]"><Icon className="h-4 w-4" /> {meta.eyebrow}</div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="mb-6 flex items-end justify-between border-b-2 border-black pb-3">
          ...
          <span className="mb-1 flex items-center gap-2 text-xs font-bold text-[#032a72]"><Sparkles className="h-4 w-4" /> 持续更新</span>
        </div>
        ...
        {posts.length > 0 ? (
              ...
              <section className="mt-10 grid gap-x-8 gap-y-0 border-t border-[#dfe3dc] md:grid-cols-2">
                {posts.slice(3).map((post) => (
                  <Link key={post.id} href={href(post)} className="group flex items-start justify-between gap-5 border-b border-[#dfe3dc] py-6">
                    <div><p className="text-xs font-bold text-[#032a72]">{categoryLabel(post.category)}</p><h3 className="mt-2 text-xl font-black leading-snug group-hover:text-[#032a72]">{post.title}</h3><p className="mt-2 text-sm text-[#606b80] line-clamp-2">{post.excerpt}</p></div>
                    <ArrowRight className="mt-1 h-5 w-5 shrink-0 text-[#4870ff]" />
                  </Link>
                ))}
              </section>
            )}
          ...
          <div className="border border-dashed border-[#cfd4cc] bg-[#fafbf9] py-20 text-center">
            <Icon className="mx-auto h-9 w-9 text-[#4870ff]" />
